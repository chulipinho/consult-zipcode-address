package com.example.consultaFrete.docs;

import java.util.ArrayList;
import java.util.List;

import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;
import org.springframework.web.bind.annotation.RequestMethod;

import springfox.documentation.builders.ApiInfoBuilder;
import springfox.documentation.builders.PathSelectors;
import springfox.documentation.builders.RequestHandlerSelectors;
import springfox.documentation.builders.ResponseMessageBuilder;
import springfox.documentation.service.ApiInfo;
import springfox.documentation.service.Contact;
import springfox.documentation.service.ResponseMessage;
import springfox.documentation.spi.DocumentationType;
import springfox.documentation.spring.web.plugins.Docket;
import springfox.documentation.swagger2.annotations.EnableSwagger2;

@Configuration
@EnableSwagger2
public class SwaggerConfig {
    @Bean
    public Docket api() {
        return new Docket(DocumentationType.SWAGGER_2)
          .select()
          .apis(RequestHandlerSelectors.basePackage("com.example.consultaFrete.address"))
          .paths(PathSelectors.any())
          .build()
          .useDefaultResponseMessages(false)
          .globalResponseMessage(RequestMethod.GET, defaultResponseMessage())
          .globalResponseMessage(RequestMethod.POST, defaultResponseMessage())
          .apiInfo(apiInfo());
    }

    private List<ResponseMessage> defaultResponseMessage() {
        return new ArrayList<ResponseMessage>() {{
            add(new ResponseMessageBuilder()
                .code(500)
                .message("Erro interno do servidor")
                .build());
            add(new ResponseMessageBuilder()
                .code(400)
                .message("Parâmetro de entrada inválido")
                .build()
            );
            add(new ResponseMessageBuilder()
                .code(404)
                .message("Endereço não encontrado")
                .build()
            );
        }};
    }

    private ApiInfo apiInfo() {
        return new ApiInfoBuilder()
                    .title("Api para consulta de fretes")
                    .description("Aplicação REST em Spring Boot")
                    .version("1.0.0")
                    .contact(new Contact("Fellipe Machado", "https://github.com/chulipinho", "fellipe.luz.machado@gmail.com"))
                    .build();
    }
}
